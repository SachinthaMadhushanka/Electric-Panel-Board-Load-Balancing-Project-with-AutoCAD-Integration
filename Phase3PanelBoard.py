...
def assign_circuits(circuits, index, phases, phase_power, best_phases, best_diff, memo, max_diff=4, max_spaces=22):
    ...
    for phase_combination in combinations(['A', 'B', 'C'], circuit_phases):
...
def distribute_phases(circuits, max_spaces):
    ...
    print("Start Distributing Among 3 Phases........")

    # circuits = sorted(circuits, key=lambda x: (-x[1], x[2]))
    phases = {'A': [], 'B': [], 'C': []}
    phase_power = {'A': 0, 'B': 0, 'C': 0}
    memo = {}
...
def readDataFromFile(input_filename):
    ...
    for row in range(3, sheet.max_row + 1):
        total_power_list.append(
            (row, sheet.cell(row=row, column=total_power_col).value, sheet.cell(row=row, column=poles_col).value))
...
def WriteRow(sheet, row_id, is_empty, last_col_index=0, excel_data=None, index_excel=0, phase_power=0, phase_col=1):
    ...
    # Insert Pole 1 load in Phase A to the current row (Then Delete that load from pole_1_lists)
        ...
        for col in range(1, last_col_index):
            ...
            if not new_data:
                new_data = ""
            sheet.cell(row=row_id, column=col + 3).value = new_data

        # Set Circuit name data at the end
        sheet.cell(row=row_id, column=last_col_index + 3).value = excel_data[index_excel][0]

        # Set Phase Power (In one of the first 3 cols)
        sheet.cell(row=row_id, column=phase_col).value = phase_power


def generateOutputFile(work_book, pole_1_lists, pole_2_lists, pole_3_lists, excel_data, output_filename):
    """
    ...
    sheet.cell(row=2, column=3).value = "Phase C"
    ...
    mergeCells(sheet, (1, last_col_index + 3), (2, last_col_index + 3))
    sheet.cell(row=1, column=last_col_index + 3).value = "CIRCUIT LIST"

    mergeCells(sheet, (1, last_col_index + 2), (2, last_col_index + 2))

    table_index = 2
    # Write pole 3 circuits data to the output file
    for sequence_num in range(len(pole_3_lists[0])):

        # Row location of the table
        table_index = sequence_num * 3 + 3

        # Get the index of the next circuit (To access excel data)
        index_of_next_circuit = pole_3_lists[0][sequence_num][0]

        # Merge 3 rows for load columns, total_power_columns and circuit_name column
        for col in range(4, sheet.max_column + 1):
            mergeCells(sheet, (sequence_num * 3 + 3, col), (sequence_num * 3 + 5, col))

        # Set circuit name
        sheet.cell(row=table_index, column=last_col_index + 3).value = excel_data[index_of_next_circuit - 3][0]
        ...
        for col in range(1, last_col_index):
            ...
            if not new_data:
                new_data = ""
            sheet.cell(row=table_index, column=col + 3).value = new_data

        # Set Phase Power
        # For each Phase (Phase A, B, C)
        for list_index in range(3):
            table_index = sequence_num * 3 + list_index + 3

            # Set total power column (In one of the 3 Phases)
            sheet.cell(row=table_index, column=list_index + 1).value = pole_3_lists[list_index][sequence_num][1]
    ...
    #
    # pole_1_lists = [[], [], []]
    # pole_2_lists = [[(3, 0), (4, 500), (5, 600.0)], [(3, 200), (4, 500), (5, 0)], [(3, 200), (4, 0), (5, 600.0)]]
    # pole_3_lists = [[(7, 3750.0), (8, 2000.0), (6, 600.0)], [(7, 3750.0), (8, 2000.0), (6, 600.0)],
    #                 [(7, 3750.0), (8, 2000.0), (6, 600.0)]]

    # Sort pole_2_lists and add 0 load to the other phase
    # pole_2_lists = [[(3, 0), (4, 500), (5, 600.0)], [(3, 200), (4, 500), (5, 0)], [(3, 200), (4, 0), (5, 600.0)]]
    all_2_pole_ids = list(set([load_data[0] for lst in pole_2_lists for load_data in lst]))
    for lst in pole_2_lists:
        for pole_2_id in all_2_pole_ids:
            if pole_2_id not in [load_data[0] for load_data in lst]:
                lst.append((pole_2_id, 0))
    ...
    while len(pole_2_lists[0]) != 0:
        load_index, gaps = getNext2PoleLoadIndex(pole_2_lists, next_phase_start_index)

        phase_BC = pole_2_lists[0][load_index][1] == 0
        phase_AC = pole_2_lists[1][load_index][1] == 0
        phase_AB = pole_2_lists[2][load_index][1] == 0
        ...
        for i in range(gaps):
            gap_line_index = (next_phase_start_index + i - 1) % 3

            # If there is no 1 Pole load for that phase
            if len(pole_1_lists[gap_line_index]) == 0:
            ...
            # Insert Pole 1 load in that Phase to the current row (Then Delete that load from pole_1_lists)
                ...
                index_of_next_circuit = pole_1_lists[gap_line_index][0][0]
                ...
                WriteRow(sheet=sheet, row_id=table_index, is_empty=False, last_col_index=last_col_index,
                         ...
                         phase_power=pole_1_lists[gap_line_index][0][1], phase_col=gap_line_index + 1)

                # Deleting Already assigned load
                del pole_1_lists[gap_line_index][0]

            table_index += 1

        """
            Put distributed Phase Power
        """

        # Merge 3 rows for load columns, total_power_columns and circuit_name column
        for col in range(4, sheet.max_column + 1):
            mergeCells(sheet, (table_index, col), (table_index + 1, col))

        index_of_next_circuit = pole_2_lists[0][load_index][0]
        ...
        if phase_AB:
            ...
            for phase in [0, 1]:
                # Set total power column (In one of the 3 Phases)
                sheet.cell(row=table_index, column=phase + 1).value = pole_2_lists[phase][load_index][1]
                table_index += 1

            next_phase_start_index = 3
        ...
        # Delete Pole 2 Load from pole_2_lists
        for i in range(3):
            del pole_2_lists[i][load_index]

    """
        Write 1 Pole Loads
    """

    while len(pole_1_lists[0]) != 0 or len(pole_1_lists[1]) != 0 or len(pole_1_lists[2]) != 0:
        if next_phase_start_index == 4:
            next_phase_start_index = 1
...
def balance3PhasePanelBoard(input_filename, output_filename, max_circuits):
    ...
    for lst in sorted_phase_list:
        for circuit_index, power in lst:
            ...
            if poles == 1:
                ...
                pole_3_lists[phase_index].append((circuit_index, power))

        phase_index += 1

    # generateOutputFile(work_book, best_phases, excel_data, output_filename)
    generateOutputFile(work_book, pole_1_lists, pole_2_lists, pole_3_lists, excel_data, output_filename)

    return best_phases

# TA_input_filename = "Circuit List - TA.xlsx"
# TA_output_filename = "TB.xlsx"
# max_circuits = 16
#
# best_phases_TA = balance3PhasePanelBoard(TA_input_filename, TA_output_filename, max_circuits)
